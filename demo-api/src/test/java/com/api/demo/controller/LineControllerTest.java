...
class LineControllerTest {
    ...
    void createLine_returns201() {
        ...
        ResponseEntity<ApiResponse<LineDTO>> response = lineController.createLine(lineDTO);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(response.getBody().getData().getName()).isEqualTo("Auto");
    }
    ...
    void getLineById_returns200() {
        ...
        ResponseEntity<ApiResponse<LineDTO>> response = lineController.getLineById(1);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody().getData().getMaxCoverage()).isEqualTo(500000f);
    }
    ...
    void getAllLines_returns200() {
        ...
        ResponseEntity<ApiResponse<List<LineDTO>>> response = lineController.getAllLines();
    ...
    void updateLine_returns200() {
        when(lineService.partialUpdate(1, lineDTO)).thenReturn(lineDTO);

        ResponseEntity<ApiResponse<LineDTO>> response = lineController.updateLine(1, lineDTO);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody().getData().getDescription()).isEqualTo("Automobile insurance coverage");
    }
    ...
    void deleteLine_returns200() {
        ...
        ResponseEntity<ApiResponse<String>> response = lineController.deleteLine(1);
